...
* Une partie Test où nous testons l'algorithme d'optimisation de maximisation de l'entropie (ou de minimisation de l'entropie croisée)  et nous évaluons la solution trouvées
...
* Nous avons eu des erreurs avec minxen.
...
goodness_of_fit <- rep(-1,length(iteration_to_analyse))
divergence <- rep(0,length(iteration_to_analyse))
compare_eta_minxent <- rep(-1,length(iteration_to_analyse))
compare_proba_L2<- rep(-1,length(iteration_to_analyse))
Rdeux<- rep(-1,length(iteration_to_analyse))
compare_proba_L1<- rep(-1,length(iteration_to_analyse))
o<-0
for (iteration_max in iteration_to_analyse) {
  ...
  r_proba <- probability_estimate/proba_true
  ...
  goodness_of_fit[o] <- t(df_proba)%*%dfr_proba
  divergence[o] <- t(probability_estimate)%*%log(r_proba)
...
o<-o+1
r_proba_simplex <- proba_simplex/proba_true
...
* compare_proba_L2 : 85 % de  la norme est expliqué
*  compare proba L1 ;  on fait une erreur de 15% en moyenne sur les proba réelles, le simplexe de 100% ce qui justifie bien notre critère de minimisation
*  goodness of fit : à interpréter du point de vue du khi deux, mais  nous allons trouver une p-value de j'imagine 3%
*  divergence : à interpréter, le simplexe part à l'infini car il a beaucoup de probabilité à zéro
*  compare eta minxent : erreur sur les marginaux de 2%, c'est quatre fois mieux que le simplexe qui est pourtant conçu pour trouver une solution compatible
