...
# Travail en cours sur Minxent

Le calcul du Hessien semble compliqué malgré la robustification...
Error in if (any(wt < 0) || (s <- sum(wt)) == 0) stop("weights must be non-negative and not all zero") : 
  ...
  Mon analyse est que c'est compliqué de calculer un Hessien avec 1200 x 1200. Il vaut mieux laisser tomber et utiliser notre méthode plus robuste qui est basé sur le gradient : la méthode des petits pas 
...
install.packages("minxent") Minxent n'est plus maintenu dans le CRAN, c'est pourquoi j'ai téléchargé la dernière version
 ...
 minxent.multiple_pov_back<-function (q, G, eta,lambda)
 {
 fk<-function (lambda, q, G, eta)
 {
      lambda0<-log(sum(q*exp(-lambda%*%G[-1,])))
